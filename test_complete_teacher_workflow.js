// Complete Teacher Workflow Test - API + Dashboard
// Date: 2025-10-21
...
const TEACHER_DATA = {
  name: 'Ahmed Hassan Al-Qahtani',
  email: `ahmed.test.${timestamp}@quranakh.test`,
  password: 'Teacher123456!',
  phone: '+1234567890',
  qualifications: 'Ijazah in Hafs, Master in Islamic Studies',
  experience: '8 years teaching Quran and Tajweed'
};
...
async function completeTeacherWorkflowTest() {
  ...
  let teacherEmail = TEACHER_DATA.email;
  let teacherPassword = null;

  // PHASE 1: API ACCOUNT CREATION
  console.log('📡 PHASE 1: Create Teacher via API');
  console.log('-'.repeat(70));
  ...
  try {
    ...
    // Try /api/auth/create-teacher endpoint (used by TeacherManagementV2)
    console.log('\n🔧 Step 1.2: Call /api/auth/create-teacher...');
    ...
    if (result.success || response.ok) {
      ...
      if (result.credentials) {
        ...
        console.log(`   Password: ${result.credentials.password || TEACHER_DATA.password}`);
        console.log(`   Login URL: ${result.credentials.loginUrl || TEST_URL + '/login'}`);

        // Store the actual password for Phase 3
        teacherPassword = result.credentials.password;
      }
    } else {
  ...
  if (results.apiCreation && results.databaseRecord) {
    ...
    try {
      ...
      console.log(`   Password: ${teacherPassword || 'NOT SET - WILL FAIL'}`);
      await page.type('input[type="email"]', teacherEmail, { delay: 50 });
      await page.type('input[type="password"]', teacherPassword || TEACHER_DATA.password, { delay: 50 });
      ...
      await page.screenshot({
        path: './.playwright-mcp/teacher-dashboard-full.png',
        fullPage: true
      });
      console.log('   ✅ Full page: teacher-dashboard-full.png');

      await page.screenshot({
        path: './.playwright-mcp/teacher-dashboard-viewport.png'
      });
      console.log('   ✅ Viewport: teacher-dashboard-viewport.png');
