...
interface Point {
  x: number;  // These will be in PDF coordinate space (0-1 normalized)
  y: number;  // These will be in PDF coordinate space (0-1 normalized)
}
...
// Individual page component with zoom-persistent annotations
const PageCanvas = memo(({ 
  ...
  useEffect(() => {
    const renderPage = async () => {
      ...
      try {
        ...
        // Store the actual PDF page dimensions at scale 1
  ...
  // Convert screen coordinates to PDF coordinates (0-1 normalized)
  const screenToPdfCoords = (screenX: number, screenY: number): Point => {
    // Normalize to 0-1 range based on PDF dimensions
    return {
      x: screenX / (pdfDimensions.width * currentScale),
      y: screenY / (pdfDimensions.height * currentScale)
    };
  };

  // Convert PDF coordinates (0-1 normalized) to screen coordinates
  const pdfToScreenCoords = (pdfX: number, pdfY: number): { x: number; y: number } => {
    return {
      x: pdfX * pdfDimensions.width * currentScale,
      y: pdfY * pdfDimensions.height * currentScale
    };
  };
  ...
  const getToolWidth = () => {
    // Scale stroke width with zoom to maintain visual consistency
    const baseWidth = tool === 'yellow_highlight' ? strokeWidth * 4 : strokeWidth;
    return baseWidth * (currentScale / 2); // Adjust for the base scale of 2
  };

  // Drawing handlers with coordinate conversion
  ...
  const draw = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    // Draw on canvas (convert back to screen coords for display)
  ...
  const endDrawing = () => {
    ...
    if (currentStroke.length > 1 && tool) {
      // Store stroke with PDF coordinates
      const newStroke: Stroke = {
        points: currentStroke, // Already in PDF coords
        color: getToolColor(),
        width: strokeWidth, // Store original width, not scaled
        tool: tool
      };
  ...
  const handleClick = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    // Convert to PDF coordinates for comparison
    const pdfPoint = screenToPdfCoords(screenX, screenY);
    
    // Find and remove stroke near click point
    const threshold = 15 / (pdfDimensions.width * currentScale); // Scale threshold
  ...
  // Redraw strokes with proper coordinate conversion
  const redrawStrokes = () => {
    const canvas = drawCanvasRef.current;
    if (!canvas) return;
    ...
    strokes.forEach(stroke => {
      ...
      // Scale width based on current zoom
      ctx.lineWidth = stroke.tool === 'yellow_highlight' 
        ? stroke.width * 4 * (currentScale / 2)
        : stroke.width * (currentScale / 2);
      ctx.lineCap = 'round';
      ctx.lineJoin = 'round';
  ...
  // Redraw when strokes change or zoom changes
  useEffect(() => {
    if (isRendered) {
      redrawStrokes();
    }
  }, [strokes, isRendered, currentScale]);
...
export default function ZoomPersistentViewer({ pdfUrl, studentId = 'demo' }: Props) {
  // State
  ...
  useEffect(() => {
    const initPdf = async () => {
      ...
      console.log('✅ PDF.js initialized with zoom persistence');
      loadPdf();
    };
  ...
  const handleStrokeComplete = (pageNum: number, stroke: Stroke) => {
    if (stroke.tool.startsWith('remove_')) {
      ...
      console.log(`💾 Stroke saved in PDF coordinates for page ${pageNum}`);
    }
  };
              ...
              <div className="flex items-center gap-2">
                <button
                  onClick={() => {
                    ...
                    console.log('🔍 Zoom changed, annotations will stay in place');
                  }}
