// Complete Parent Workflow Test - API + Dashboard
// Date: 2025-10-21
...
const PARENT_DATA = {
  name: 'Amina Mohammed Al-Rahman',
  email: `amina.test.${timestamp}@quranakh.test`
};
...
async function completeParentWorkflowTest() {
  console.log('\n🎯 COMPLETE PARENT WORKFLOW TEST');
  ...
  let parentEmail = PARENT_DATA.email;
  let parentPassword = null;

  // PHASE 1: API ACCOUNT CREATION
  console.log('📡 PHASE 1: Create Parent via API');
  console.log('-'.repeat(70));
  ...
  try {
    ...
    // Call /api/auth/create-parent endpoint (mirrors teacher/student endpoint pattern)
    console.log('\n🔧 Step 1.2: Call /api/auth/create-parent...');

    const response = await fetch(`${TEST_URL}/api/auth/create-parent`, {
      method: 'POST',
      headers: {
    ...
    if (result.success || response.ok) {
      ...
      if (result.credentials) {
        ...
        parentPassword = result.credentials.password;
        parentEmail = result.credentials.email;
      }
    } else {
  ...
  if (results.apiCreation && results.databaseRecord) {
    ...
    try {
      ...
      console.log(`   Password: ${parentPassword || 'NOT SET - WILL FAIL'}`);
      await page.type('input[type="email"]', parentEmail, { delay: 50 });
      await page.type('input[type="password"]', parentPassword, { delay: 50 });
      ...
      await page.screenshot({
        path: './.playwright-mcp/parent-dashboard-full.png',
        fullPage: true
      });
      console.log('   ✅ Full page: parent-dashboard-full.png');

      await page.screenshot({
        path: './.playwright-mcp/parent-dashboard-viewport.png'
      });
      console.log('   ✅ Viewport: parent-dashboard-viewport.png');
